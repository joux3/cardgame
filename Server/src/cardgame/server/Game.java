...
public abstract class Game {
	...
	public final void incomingPacket(Player sender, PacketParser packet) {
		System.out.println("Got packet " + packet.getPacketName() + " from "
				+ sender.getPlayerName());
		if (packet.getPacketName().equals("gamechat")) {
			String message = packet.getString();
			System.out.println("Player " + sender.getPlayerName() + " (game "
					+ this.gameName + ") said: " + message);
	...
	public void playerLeft(Player disconnecter) {
		...
		if (!players.remove(disconnecter)) {
			throw new RuntimeException(
					"Player who isn't in the player list left!");
		}
